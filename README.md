...
> Never commit your `.env` file or expose API keys, database credentials, JWT secrets, or Razorpay secrets publicly.
...
```bash
git clone https://github.com/Prasad-Aki/AI_WEBSITE_BUILDER.git
...
The frontend will run locally using Vite and communicate with the Express backend.
...
```text
Website generation and management endpoints
```
